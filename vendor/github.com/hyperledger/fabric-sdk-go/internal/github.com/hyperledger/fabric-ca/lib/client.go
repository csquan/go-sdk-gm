...
func (c *Client) initHTTPClient(serverName string) error {
	...
	c.Config.TLS.Enabled = false
	if c.Config.TLS.Enabled {
		log.Info("TLS Enabled")
		tlsConfig, err2 := tls.GetClientTLSConfig(&c.Config.TLS, c.csp)
		if err2 != nil {
			fmt.Println("====Failed to get client TLS config err:=====")
			fmt.Println(err2)
			fmt.Println("====Client:=====")
			fmt.Println(c)
...
func (c *Client) GenCSR(req *api.CSRInfo, id string) ([]byte, core.Key, error) {
	...
	if (cr.KeyRequest == nil) || (cr.KeyRequest.Size() == 0 && cr.KeyRequest.Algo() == "") {
		basicKeyRequest := newCfsslBasicKeyRequest(api.NewBasicKeyRequest())
		if &basicKeyRequest == nil {
			log.Info("basicKeyRequest = nil")
			return nil, nil, err
		}
		log.Debugf("basicKeyRequest %+v", basicKeyRequest)
		cr.KeyRequest = csr.NewKeyRequest()
		log.Debugf("basicKeyRequest.BA %+v", basicKeyRequest.BA) //"ecdsa"
		log.Debugf("basicKeyRequest.BS %+v", basicKeyRequest.BS) //256
		cr.KeyRequest.A = basicKeyRequest.BA
		cr.KeyRequest.S = basicKeyRequest.BS
	}
...
func (c *Client) newCertificateRequest(req *api.CSRInfo) *csr.CertificateRequest {
	...
	if req != nil && req.KeyRequest != nil {
		basicKeyRequest := newCfsslBasicKeyRequest(req.KeyRequest)
...
func newCfsslBasicKeyRequest(bkr *api.BasicKeyRequest) *csr.BasicKeyRequest {
	return &csr.BasicKeyRequest{BA: bkr.Algo, BS: bkr.Size}
}
